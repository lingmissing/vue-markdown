      ...
      <div
        ...
        <div class="content-input-wrapper">
          ...
          <v-autoTextarea 
            ...
            lineHeight="1.5" 
            v-model="d_value"></v-autoTextarea>
        </div>
      </div>
        <!--展示区-->
        <div
  ...
  export default {
    ...
    created() {
      ...
      markdown.renderAsync = function(src, env, fuc, _env) {
        env = env || {}
        _env = _env || {}
        var _res = markdown.renderer.render(
          this.parse(src, env),
          this.options,
          env
        )
        if (_env['ishljs'] === false) fuc(_res)
          ...
          hljs(
            _res,
            fuc,
            $vm.p_external_link.hljs_lang,
            $vm.p_external_link.hljs_js
          )
        }
      }
      // 初始化语言
    ...
    methods: {
      ...
      initExternalFuc() {
        ...
        for (var i = 0; i < _external_.length; i++) {
          if (
            (_is_boolean && !$vm.external_link) ||
            (_is_object && $vm.external_link[_external_[i]] === false)
          ) {
            $vm.p_external_link[_external_[i]] = false
          } else if (
            _is_object &&
            typeof $vm.external_link[_external_[i]] === 'function'
          ) {
      ...
      initLanguage() {
        let lang =
          CONFIG.langList.indexOf(this.language) >= 0
            ? this.language
