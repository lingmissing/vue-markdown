...
function highLightCode(str, callback, hljsLangFuc, hljsFuc) {
  ...
  if (!hljs) {
    if (typeof hljsFuc !== 'function') {
      console.warn(
        'external_link.hljs_js is not a function, hljs can not load by mavon-editor, if you want to disabled this log, set external_link.hljs_js to function'
      )
      callback(str)
      return
    }
    var url = hljsFuc()
    console.warn(
      'hljs parsing file is missing. mavon-editor will autoload',
      url
    )
    ...
    if (typeof hljsLangFuc === 'function') {
      ...
      console.warn(
        'external_link.hljs_lang is not a function, hljs will not to work'
      )
      callback(str)
    }
  }
    ...
    for (i = 0; i < pre_code.length; i++) {
      ...
      if (lang.length > 0) {
        ...
        if (
          ...
          !not_include_lang.hasOwnProperty(lang) &&
          url.length > 0
        ) {
